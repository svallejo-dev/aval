...
func TestParseFixtures(t *testing.T) {
	...
	tests := []struct {
		...
		status   map[string]evidence.Status // includes IDs no test carries
		warnings []Warning
	}{{
...
func TestParseEdgeCases(t *testing.T) {
	...
	wantTests := []string{"TestA=not_run", "TestB=pass", "TestB/ORD-F01_x=pass", "TestB/ORD-F01_x/ORD-F03_y=skipped"}
	...
	if g := r.Status(id(t, "ORD-F09")); g != evidence.NotRun {
		t.Errorf("Status(ORD-F09) = %s, want not_run: some packages built", g)
	}
