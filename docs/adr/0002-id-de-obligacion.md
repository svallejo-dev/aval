...
4. **Todo `INFO` de `validate --strict` es un fallo.** OpenSpec devuelve `valid: true` y exit 0 con issues que anuncian que archive fallará o que parte del delta se ignora, por ejemplo *"Archive would refuse this delta"* o *"… is not a "### Requirement:" header and is ignored by validation"*. Además, aval prohíbe cualquier cabecera de nivel 1 a 3 que no sea `### Requirement:`, también sin texto, dentro de las secciones de requirements: `## Requirements` en las specs y las cuatro secciones de los deltas. Los lectores de OpenSpec no coinciden en dónde acaba el requirement: un `# Aside` dentro de un ADDED pasa `validate --strict` y archive lo rechaza; entre dos ADDED, archive lo escribe en la spec y `show` pierde el requirement siguiente (fixtures `h1-in-requirement` y `h1-between-requirements`).
