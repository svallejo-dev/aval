...
2. **Definición y referencia.**
   ...
   - Los **IDs retirados** son los que aparecen en bloques REMOVED de `openspec/changes/archive/*/specs/` y los que un RENAMED archivado dejó atrás al cambiar de ID. Nunca se reutilizan: definir un ID retirado, en una spec principal o en un ADDED, es un error.
   - Un change **archivado en el mismo PR** que el código se revisa entero, como si siguiera activo, contra las specs principales del commit base del PR, que son las de antes del archive. No se reconstruyen desde head: un MODIFIED sustituye el bloque entero y con él se pierden los escenarios y la marca que hay que comparar.
...
4. **Todo `INFO` de `validate --strict` es un fallo, salvo una lista explícita de excepciones.** OpenSpec devuelve `valid: true` y exit 0 con issues que anuncian que archive fallará o que parte del delta se ignora, por ejemplo *"Archive would refuse this delta"* o *"… is not a "### Requirement:" header and is ignored by validation"*.
   - Las excepciones se comparan por el texto exacto del mensaje, solo con nivel INFO, y viven en el código (`allowedInfo` en `internal/openspec`). Añadir una exige enmendar esta regla.
   - Hoy hay una sola: *"skip_specs is set in .openspec.yaml: change declares no spec-level behavior changes, zero deltas accepted"*. OpenSpec la emite cuando un change declara `skip_specs` y no trae deltas. Ese change no tiene obligaciones que validar ni nada que archive pueda aplicar mal, así que tratarlo como fallo solo obligaría a inventar un delta.

   Además, aval prohíbe cualquier cabecera de nivel 1 a 3 que no sea `### Requirement:`, también sin texto, dentro de las secciones de requirements: `## Requirements` en las specs y las cuatro secciones de los deltas. Los lectores de OpenSpec no coinciden en dónde acaba el requirement: un `# Aside` dentro de un ADDED pasa `validate --strict` y archive lo rechaza; entre dos ADDED, archive lo escribe en la spec y `show` pierde el requirement siguiente (fixtures `h1-in-requirement` y `h1-between-requirements`).
