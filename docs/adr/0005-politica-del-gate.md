...
- **head** es `github.event.pull_request.head.sha`, y el checkout usa `ref:` head, no el merge commit. Dentro de Actions con un PR en el evento, `--trust-base`, `--change-base` y `--head` se rechazan (exit 2): el evento manda.
...
- **La base de confianza se mueve:** es el tip en el momento de la ejecución, así que reejecutar el gate sobre el mismo head puede dar otro veredicto si la rama por defecto avanzó. Es lo que se quiere —manda la política vigente— y el `trustBase` del bundle (§7) dice con cuál se decidió.
